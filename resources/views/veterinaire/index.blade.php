    ...
    <tbody>
        ...
        <tr>
            ...
            <td> @inject('provider', 'App\Http\Controllers\OvinController')
                {{ $provider::age($ovin->date_naissance, $ovin->die_date)[4] }}
            </td>
            @else
            <td>{{ $ovin->date_naissance }} و</td>
            <td> @inject('provider', 'App\Http\Controllers\OvinController')
                {{ $provider::age($ovin->date_naissance, date('Y-m-d'))[4] }}
            </td>
            @endif
            ...
            <td><a class="btn btn-primary" href="{{ route('ovins.edit', $ovin->id) }}" role="button">تعديل</a>
                ...
                <a class="btn btn-success" href="{{ route('ovins.naissance', $ovin->id) }}" role="button">ولادة</a>
